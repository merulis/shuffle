...
func (c *Client) GetListContent(
	...
	var items []entity.ContentItemGH
...
func (c *Client) GetFileContent(
	...
	var file entity.FileContentGH
	if err := json.Unmarshal(raw, &file); err != nil {
		return nil, fmt.Errorf("decode response body: %w", err)
	}

	if file.Type != string(entity.ArtifactFile) {
		return nil, fmt.Errorf("path %q is not a file, got type %q", repoPath, file.Type)
	}

	if file.Encoding != string(entity.ArtifactBase64) {
